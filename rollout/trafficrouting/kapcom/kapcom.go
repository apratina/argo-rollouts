...
// ReconcilerConfig describes static configuration data for the kapcom reconciler
...
// Reconciler holds required fields to reconcile Kapcom resources
...
// Type indicates this reconciler is an Kapcom reconciler
...
func (r *Reconciler) canaryIngress(stableIngress *extensionsv1beta1.Ingress, name string, desiredWeight int32) (*extensionsv1beta1.Ingress, error) {
	stableIngressName := r.cfg.Rollout.Spec.Strategy.Canary.TrafficRouting.Kapcom.StableIngress
	...
	for k, v := range r.cfg.Rollout.Spec.Strategy.Canary.TrafficRouting.Kapcom.AdditionalIngressAnnotations {
...
// SetWeight modifies Kapcom Ingress resources to reach desired state
...
func (r *Reconciler) SetWeight(desiredWeight int32) error {
	...
	r.log.Infof("Entered SetWeight Method.................")

	const annotation = "kubernetes.io/ingress.class"
	var apexName = "rollouts-demo"
				...
				Services: []v1beta1.Service{
					{
						...
						Name:   fmt.Sprintf("%s-stable", apexName),
						Port:   80,
						Weight: 100,
					...
					},
					{
						// Name:   canaryName,
						// Port:   int(canary.Spec.Service.Port),
						Name:   fmt.Sprintf("%s-canary", apexName),
						Port:   80,
						Weight: 0,
						// HealthCheck: kr.makeHealthCheck(canary),
					},
	...
	_, err := r.cfg.Client.KapcomV1beta1().IngressRoutes(namespaceName).Get(context.TODO(), apexName, metav1.GetOptions{})

	if errors.IsNotFound(err) {
		igr := &v1beta1.IngressRoute{
			ObjectMeta: metav1.ObjectMeta{
				Name:      apexName,
		...
		igr.Annotations = map[string]string{
			annotation: "contour-corp",
		}

		_, err = r.cfg.Client.KapcomV1beta1().IngressRoutes(namespaceName).Create(context.TODO(), igr, metav1.CreateOptions{})
		if err != nil {
			return fmt.Errorf("IngressRoute %s.%s create error: %w", apexName, namespaceName, err)
		}

		r.log.Infof("IngressRoute %s.%s created", igr.GetName(), namespaceName)

		return nil

	} else if err != nil {
		return fmt.Errorf("IngressRoute %s.%s get query error: %w", apexName, namespaceName, err)
	}
